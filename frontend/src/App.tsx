            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  ...
                  >
                    <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-blue-600 font-bold">{user ? user.name.slice(0, 2).toUpperCase() : 'AR'}</span>
                    <span className="hidden max-w-32 truncate sm:inline">{user ? user.name : 'Account'}</span>
                  ...
                  {userMenuOpen && (
                    <div className="absolute right-0 z-50 mt-3 w-48 rounded-xl border border-slate-200 bg-white p-2 text-sm text-slate-600 shadow-xl">
                      ...
                      >
                        ...
                        Logout
                      </button> : <button type="button" onClick={() => { setUserMenuOpen(false); navigate('/login'); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-blue-600 transition hover:bg-blue-50">Sign in</button>}
                    </div>
                  )}
                </div>
              </div>
    ...
    <div className={`min-h-screen bg-slate-100 ${isHomeRoute ? 'pt-0' : 'pt-24 lg:pt-28'}`}>
      <Suspense fallback={<div className="p-6">Loading...</div>}>
        <Routes>
          ...
          <Route path="/dashboard" element={<ProtectedRoute><Shell showSidebar={true}><Dashboard /></Shell></ProtectedRoute>} />
          <Route path="/planner" element={<ProtectedRoute><Shell showSidebar={true}><Planner /></Shell></ProtectedRoute>} />
          <Route path="/resources" element={<ProtectedRoute><Shell showSidebar={true}><ResourceLibrary /></Shell></ProtectedRoute>} />
          <Route path="/product-tour" element={<ProtectedRoute><Shell showSidebar={true}><ProductTour /></Shell></ProtectedRoute>} />
          <Route path="/meeting-agenda" element={<ProtectedRoute><Shell showSidebar={true}><MeetingAgenda /></Shell></ProtectedRoute>} />
          <Route path="/contact" element={<Shell showSidebar={true} showFloatingCalculator={false}><Contact /></Shell>} />
          <Route path="/support/ticket" element={<ProtectedRoute><Shell showSidebar={false} showFloatingCalculator={false}><SupportTicket /></Shell></ProtectedRoute>} />
          ...
          <Route path="/profile" element={<ProtectedRoute><Shell showSidebar={true} showFloatingCalculator={false}><Profile /></Shell></ProtectedRoute>} />
          <Route path="/settings" element={<ProtectedRoute><Shell showSidebar={true} showFloatingCalculator={false}><SettingsPage /></Shell></ProtectedRoute>} />
